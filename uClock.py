...
try:
    class Tool(NoIconFramelessWindow,Ui_tool.Ui_Form):
        ...
        def __init__(self):
            ...
            self.setup()
            self.ads.connect(lambda:(self.famous.adjustSize(),self.adjustSize()))
        def setup(self):
            try:
                with open("settings.json","r",encoding="utf-8") as f:
                    self.settings=json.load(f)
            except:
                self.settings={"appearance": {"mode": "effect", "pic": "", "showMode": "Window","shadow":True,"opaticy":0.90}, "countdown": {"countdownDay": "2114-05-14", "countdownThing": "先辈去世", "isCountdown": False}, "network": {"doNotTraceback": False}, "other": {"startupOnLogin": False}, "sidebar": {"html": "", "type": "calendar", "zoom": 0.1}}
            self.desktop = QApplication.desktop()
            self.screenRect = self.desktop.screenGeometry()
            self.height1 = self.screenRect.height()
            self.width1 = self.screenRect.width()
            self.setGeometry(int(self.width1*(1400/1920))+200,int(self.height1*(100/1080)),self.width()+50,self.height())
            ...
            if self.settings["appearance"]["mode"]=="effect":
                ...
                if self.settings["appearance"]["shadow"]:
    ...
    class ClockWindow(NoIconFramelessWindow,Ui_Form):
        ...
        def __init__(self):
            ...
            self.retranslateUi(self)
            self.setup()
            self.ads.connect(lambda:self.famous.adjustSize())
        def setup(self):
            self.desktop = QApplication.desktop()
            self.screenRect = self.desktop.screenGeometry()
            self.height1 = self.screenRect.height()
            self.width1 = self.screenRect.width()
            self.setGeometry(int(self.width1*(1400/1920)),int(self.height1*(100/1080)),self.width()+20,self.height())
            try:
                with open("settings.json","r",encoding="utf-8") as f:
                    self.settings=json.load(f)
            except:
        ...
        def testInternet(self):
            if not self.settings["network"]["doNotTraceback"]:
                try:
                    ...
                    QMessageBox.warning(self,"提示","系统未联网，可能会影响功能使用")
        def uis(self):
            ...
            self.view.urlChanged.connect(lambda x:self.view.setUrl(x))
            #win11/7圆角
        ...
        def effects(self):
            ...
            if self.settings["appearance"]["mode"]=="effect":
                if "linux" not in platform.platform().lower() and (platform.platform()>="Windows-10-10.0.19041-SP0") and (not "Windows-7" in platform.platform() and not "Windows-8" in platform.platform()):
                    self.setAttribute(Qt.WA_TranslucentBackground)
                    self.windowEffect.setAcrylicEffect(int(self.winId()),gradientColor="FCFCFC+")
    ...
    QMenu::separator {
        ...
        margin-left: 15;
                ...
                self.setWindowOpacity(self.settings["appearance"]["opaticy"])
                if self.settings["appearance"]["mode"]=="pic":
                    self.pic=True
                    try:
                        self.setStyleSheet("QMenu{color:#000000}\nQWidget{color:"+str(spider.getpjfs(self.settings["appearance"]["pic"]))+"}")
                    except:
    ...
    class allReloadThread(QThread):
        ...
        def run(self):
            ...
            while True:
                ...
                window.famous.setGeometry(0,0,window.famous.width(),0)
                window.famous.setText("")
